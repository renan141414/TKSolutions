...
def criar_banco_alunos(cliente):
    ...
    colecao_alunos.drop()  # Limpar coleção existente
    ...
    cursos = ['Engenharia', 'Medicina', 'Direito', 'Administração', 'Psicologia', 'Ciência da Computação', 'Arquitetura']
...
def consultar_alunos(cliente):
    try:
        ...
        for aluno in colecao_alunos.find():
            print(f"Nome: {aluno['nome']}\nIdade: {aluno['idade']}\nCurso: {aluno['curso']}\nNota: {aluno['nota']}\nData de Inscrição: {aluno['data_inscricao']}\n{'-' * 30}")
